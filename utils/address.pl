...
%{$settings->{print_fields}} = map {$_,1} "E-mail Address", "Mobile Phone", "Business Phone", "Home Phone", "E-mail 2 Address", "E-mail 3 Address";
%{$settings->{never_print}} = map {$_,1} 'Priority';
@{$settings->{name_fields}} = ('First Name', 'Middle Name', 'Last Name', 'Company');
print Data::Dumper->Dump([\$settings], [qw(*settings)]) if $settings->{debug};
...
sub special_sort {
    ...
    # email address next
    (($a =~ /e-mail/i) <=> ($b =~ /e-mail/i)) || (($a =~ /e-mail \d/i) <=> ($b =~ /e-mail \d/i)) || $a cmp $b;
}
